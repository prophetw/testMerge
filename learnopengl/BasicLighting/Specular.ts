...
import VSHADER_SOURCE from './Specular.vert';
import FSHADER_SOURCE from './Specular.frag';
...
function main() {
  ...
  document.title = 'diffuse,ambient,specular 漫反射+环境光+高光'
...
function redraw(gl: WebGLRenderingContext, cubePosi: VertexOptions[]){
  ...
  for(let i=0; i< cubePosi.length; i++){
    ...
    updateMVPMatrix(gl, cubePosi[i], defaultCameraPosition, perspectiveOptions)
...
function injectUI(gl: WebGLRenderingContext){
  ...
  <label for="viewX" style="width: 410px" class="form-label"> viewY: <span id="viewYV">${defaultCameraPosition.y}</span></label>
  <input type="range" class="form-range" value="${defaultCameraPosition.y}" step="0.1" min="-101" max="101" id="viewY">
  <label for="viewX" style="width: 410px" class="form-label"> viewZ: <span id="viewZV">${defaultCameraPosition.z}</span></label>
  <input type="range" class="form-range" value="${defaultCameraPosition.z}" step="0.1" min="-101" max="101" id="viewZ">
  ...
  if(viewX){
    viewX.addEventListener('change', e=>{
      if(e && e.target && e.target.value){
        console.log(' viewX ', e.target.value);
        defaultCameraPosition.x = +e.target.value
        updateUI('viewXV', ''+defaultCameraPosition.x)
        redraw(gl, cubePosi)
      }
    })
  }
  ...
  if(viewY){
    viewY.addEventListener('change', e=>{
      if(e && e.target && e.target.value){
        console.log(' viewY ', e.target.value);
        defaultCameraPosition.y = +e.target.value
        updateUI('viewYV', ''+defaultCameraPosition.y)
        redraw(gl, cubePosi)
      }
    })
  }
  ...
  if(viewZ){
    viewZ.addEventListener('change', e=>{
      if(e && e.target && e.target.value){
        console.log(' viewZ ', e.target.value);
        defaultCameraPosition.z = +e.target.value
        updateUI('viewZV', ''+defaultCameraPosition.z)
        redraw(gl, cubePosi)
      }
    })
  }
...
function updateMVPMatrix(gl: WebGLRenderingContext,
  ...
  if(gl.program === program1){
    ...
    var u_viewPos = gl.getUniformLocation(gl.program, 'viewPos');
    if (!u_viewPos) {
      console.log('Failed to get the storage location of viewPos');
      return -1;
    }
    gl.uniform3f(u_viewPos, defaultCameraPosition.x, defaultCameraPosition.y, defaultCameraPosition.z)
    ...
    var u_specularStrength = gl.getUniformLocation(gl.program, 'specularStrength');
    if (!u_specularStrength) {
      console.log('Failed to get the storage location of specularStrength');
      return -1;
    }
